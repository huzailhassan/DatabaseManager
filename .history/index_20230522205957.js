...
app.put('/:table', (req, res) => {
  ...
  // Conditions
  const json_data = requestData.conditions;
  ...
  let conditions = "WHERE " + conditions2.join(" AND ");
  console.log("conditions", conditions);


  // Updated Values
  let updatedValues = Object.entries(requestData.updatedValues).map(([columnName, columnValue]) => {
    return `${columnName}="${columnValue}"`;
  });
  
  updatedValues = `SET ${updatedValues.join(', ')}`;
  
  console.log("updated values", `UPDATE test2 ${updatedValues} ${conditions}`);
